...
import {
  layoutSequence,
  calculateParticipantXPositions,
  calculateCanvasWidth,
} from '../../src/layout/layoutSequence';
...
const twoParticipantsOneMessage: SequenceDiagramData = {
  ...
  events: [{ id: 'm1', type: 'message', from: 'p1', to: 'p2', label: 'Hello' }],
};
  ...
  describe('calculateParticipantXPositions', () => {
    it('should return correct X positions for 2 participants', () => {
      ...
      expect(positions[1]).toBe(
        DEFAULT_LAYOUT.canvasPaddingX +
          DEFAULT_LAYOUT.participantWidth +
    ...
    it('should return correct X positions for 4 participants', () => {
      ...
      for (let i = 1; i < 4; i++) {
        expect(positions[i]! - positions[i - 1]!).toBe(
          DEFAULT_LAYOUT.participantWidth + DEFAULT_LAYOUT.participantGap
        );
      }
    });
  ...
  describe('flattenEvents', () => {
    ...
    it('should handle divider rows', () => {
      const data: SequenceDiagramData = {
        schemaVersion: '1.0',
        participants: [{ id: 'p1', label: 'P1' }],
        events: [{ id: 'div1', type: 'divider', label: 'Section' }],
      };
      const normalized = normalize(data);
      const rows = flattenEvents(normalized.events);
  ...
  describe('layoutSequence', () => {
        ...
        events: [
          {
            ...
            branches: [
              {
                id: 'b1',
                events: [{ id: 'm1', type: 'message', from: 'p1', to: 'p2', label: 'Inside loop' }],
              },
            ],
          },
        ],
    ...
    it('should handle notes with correct positioning', () => {
      const data: SequenceDiagramData = {
        schemaVersion: '1.0',
        participants: [{ id: 'p1', label: 'P1' }],
        events: [{ id: 'n1', type: 'note', text: 'A note', over: ['p1'], placement: 'right' }],
      };
      const normalized = normalize(data);
      const result = layoutSequence(normalized);
